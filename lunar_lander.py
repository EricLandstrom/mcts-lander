...
class LunarLander(gym.Env):
    ...
    def set_state(self,state_in):
        assert len(state_in)==8

        pos = self.lander.position
        vel = self.lander.linearVelocity
        
        pos.x = state_in[0]*(VIEWPORT_W/SCALE/2) + VIEWPORT_W/SCALE/2
        pos.y = state_in[1]*(VIEWPORT_W/SCALE/2) + (self.helipad_y+LEG_DOWN/SCALE)
        
        vel.x = state_in[2]/((VIEWPORT_W/SCALE/2)/FPS)
        vel.y = state_in[3]/((VIEWPORT_H/SCALE/2)/FPS)
        self.lander.angle = state_in[4]
        self.lander.angularVelocity = state_in[5]/20*FPS
        
        self.legs[0].ground_contact = state_in[6]>0.0
        self.legs[1].ground_contact = state_in[7]>0.0
